PROGRAM selectbands_inclusion
  ...
  ! excl_bands as input.
  ! THIS PROGRAM takes a 2d array of data and a 2d array denoting the ranges of rows in the data
  ! to include in an output array. 
  ! The program creates the data array and uses HDF5 to write it to a file
  ! The program then reads back in the data, but uses hyperslabs to only take the data specified
  ! in the inclusion array (incl_bands).
  ! The program then writes this subset of data back to a different file.
  ! You can change the 2d array of data in the subroutine build data. Currently each element in a row is just the number
  ! of that row.
  ! You can change the inclusion matrix by changing its elements.
  ! For example, if incl_bands = [ 1, 2; 4,7; 9, 10], where ; denotes a row break,
  ! then rows 1, 2, 4, 5, 6, 7, 9, and 10 will be included in the output matrix.
  ...
  INTEGER :: nbands = 10
  INTEGER :: ncols  = 7 ! Can change this later for the testing (make this dimension very large, measure timings)
...
END PROGRAM selectbands_inclusion
